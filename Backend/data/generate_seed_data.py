...
class SeedDay:
    ...
    exercise_load: float
    nutrition_value: float  # Now in calories
...
def gen_stable(n_days: int, seed: int = 7) -> List[SeedDay]:
    ...
    base_cons = 0.86
    base_ex = 520.0
    base_nut = 2100.0  # Changed to calories
    base_rec = 82.0
    ...
    for d in iso_days(n_days):
        ...
        nut = clamp(rng.gauss(base_nut, 150.0), 1600, 2800)  # Changed to calorie range

        # Recovery: mild sensitivity, mostly stable
        rec = base_rec
        rec += (sleep - base_sleep) * 1.3
        rec += (cons - base_cons) * 10.0
        rec += (nut - base_nut) / 100.0  # Scaled for calorie impact
        ...
        days.append(
            SeedDay(
                ...
                exercise_load=round(ex, 1),
                nutrition_value=round(nut, 0),  # Round to whole calories
            )
        )
    return days
...
def gen_exercise_factor(n_days: int, seed: int = 11) -> List[SeedDay]:
    ...
    base_cons = 0.84
    base_ex = 500.0
    base_nut = 2050.0  # Changed to calories
    base_rec = 81.0
    ...
    for i, d in enumerate(all_dates):
        ...
        nut = clamp(rng.gauss(base_nut, 180.0), 1500, 2900)  # Changed to calorie range
        ...
        if i in dip_days:
            ...
            if rng.random() < 0.55:
                nut = clamp(nut - rng.uniform(250, 450), 1200, 2900)  # Reduce calories
...
def gen_sleep_factor(n_days: int, seed: int = 19) -> List[SeedDay]:
    ...
    base_cons = 0.87
    base_ex = 510.0
    base_nut = 2080.0  # Changed to calories
    base_rec = 83.0
    ...
    for i, d in enumerate(all_dates):
        ...
        nut = clamp(rng.gauss(base_nut, 170.0), 1550, 2850)  # Changed to calorie range
...
def main() -> None:
    n_days = 60  # change to 45–90 if you want
    out_dir = Path("data")
